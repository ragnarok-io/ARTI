...
class CustomSurvival(nn.Module):
    def forward(self, x):
...
def test_web_export_writes_hashed_artifact(tmp_path, module, inputs, output_shape):
    ...
    assert (
        manifest["module"]["component_provenance"]["schema_version"]
        == COMPONENT_PROVENANCE_VERSION
    )
    assert manifest["module"]["component_provenance"] == component_provenance(module)
    assert manifest["module"]["state_contract"]["state_schema"]["fingerprint"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Half(stochastic=False, context_mode="contextual").eval(), "contextual Half"),
        (Half(stochastic=False, survival=CustomSurvival()).eval(), "custom survival"),
...
def test_web_export_requires_eval_float32_and_declared_inputs(tmp_path):
    with pytest.raises(ValueError, match="module.eval"):
        export(Half(stochastic=False), tmp_path / "training", example_inputs={"x": torch.randn(1, 3, 4)})
...
def test_web_contract_schema_and_generated_types_are_python_owned():
    ...
    assert "Generated by arti.experimental.web.contract" in generated
