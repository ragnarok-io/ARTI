...
def test_stage_graph_is_complete_composable_and_round_trips_strictly() -> None:
    ...
    assert graph.enabled_dependencies == (
        canonical_contract_reference("arti/half@1"),
...
def test_pulse_executor_binds_and_runs_real_half_fold_unfold() -> None:
    ...
    assert arti.component_ref(executor) == canonical_contract_reference(
    ...
    missing_dependency["components"][0]["dependencies"].remove(
        canonical_contract_reference("arti/fold@2")
    )
...
def test_only_persistable_stage_contracts_are_registered_components() -> None:
    graph = minimal_graph()
    assert arti.component_ref(graph) == canonical_contract_reference(
        "arti/pulse-stage-graph@1"
    )
    assert arti.component_ref(graph.stages[0]) == canonical_contract_reference(
        "arti/pulse-stage@1"
    )
    with pytest.raises(UnknownComponentError):
...
def test_stage_authority_comes_from_component_capabilities() -> None:
    ...
    half = registry.registration_for_reference(
        canonical_contract_reference("arti/half@1")
    )
    unfold = registry.registration_for_reference(
        canonical_contract_reference("arti/unfold@2")
    )
...
def test_stage_graph_component_spec_records_only_enabled_dependencies() -> None:
    ...
    incomplete["components"][0]["dependencies"].remove(
        canonical_contract_reference("arti/half@1")
    )
