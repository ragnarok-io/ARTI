...
neural network.
FederatedProgram remains the runtime that owns Query, path selection, local iteration,
...
snapshot, selected path, terminal ABI, operations, and finite iteration depth.
...
```python
from arti.alpha import FederalIteration, FederalResidual, FederalTopologyBlock

fold, unfold = fixed_topology.operations()
path = FederalPath(
    operations=(
        FederalTopologyBlock(fold, active_block, unfold),
        FederalIteration(iteration_block, steps=3),
        FederalResidual(residual_branch),
    ),
    source_ref="arti/federal-recall@3",
    source_snapshot_fingerprint=snapshot_fingerprint,
    path_ids=("root", "root/fold", "root/iterate", "root/residual"),
    terminal_abi_ref="arti/terminal-output-abi@1",
...
produced it. A finite `FederalIteration` is expanded as a bounded ordinary loop;
...
- `exact-static`: the route and Bank snapshot are frozen, shapes are bounded,
  state is immutable, and finite iteration can be expanded.
...
- `uncompilable`: dynamic route, data-dependent shape, mutable state, an
  unregistered callback, or unbounded iteration remains in the path.
...
Federated programs. Candidate operations inside one block must share one input and
...
iteration, and data-dependent sequence lengths without putting Python dispatch in
the graph.
...
“unbounded iteration” means open-ended with respect to the learned stop predicate,
...
`arti/federal-stateful-execution-graph@1`,
...
An attached `ARTILayer@3` artifact records its actual Federal runtime contract,
including the configured Federation identity and terminal value boundary. The
`compiled_artifact_is_separate` field remains explicit: saving an attachment
does not silently turn a dynamic Federal graph into a compiled specialization.
A Federal compiled artifact must be produced from a frozen Federal snapshot and
path evidence.
