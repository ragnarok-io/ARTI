...
`TensorInvocation` coordinates Reader iteration and tensor operations from the
...
result = invocation(
    world,
    port.resolve(),
    reader_schedule=mechanisms.ReaderIterationSchedule(reader_steps=6),
...
```text
snapshot(B_t)
  |-- Reader iteration(world_t, B_t) --> current output
