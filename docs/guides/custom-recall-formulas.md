...
Built-in Formula declarations are explicit input conveniences:
...
recall = arti.Retrieve(768, slots=32, formula="arti/state@1")
next_state = recall(hidden)
print(recall.formula_id)  # arti/state@sha256:<full-contract-digest>
```

The declaration is resolved immediately to a full SHA-256 Formula contract.
Only that resolved value is written to Formula manifests, locks, and artifact
dependencies. Old short names such as `delta-v1`, `single`, `product`, and
`state` are not aliases and are rejected.
...
recall = arti.Retrieve(768, slots=32, formula=SignedGate())
```
...
`RecallExecutor` accepts only a module declaring `output_semantics="next_state"`.
