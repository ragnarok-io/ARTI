...
underlying `BranchSearchResult`, or call the stable functions below when a host
...
ARTI K-wide branch search begins with one grouped Recall query. Its Top-K candidates
...
The runtime class/source-declaration/schema triples are strict. The source
declarations below are constructor and registry inputs; any persisted
provenance resolves them to full SHA-256 contract addresses.

| Runtime class | Source declaration | Schema |
| --- | --- | --- |
...
`run_branch_search()` consumes this bridge and runs all K candidate trajectories
...
`BranchSearchPlan@1` may additionally bind an existing
...
`BranchSearchPlan@1` defaults to `execution_layout="static_capacity"` and
...
`K-wide branch search` is breadth search over Recall candidates. One Bank query
...
permutation, ragged per-sample requested/effective `active_k`, independent per-branch iteration
...
`BranchSearchOverlayProposal` values rooted at one immutable snapshot. A
...
```python
executor = arti.mechanisms.BranchSearchExecutor.from_resident_result(result)
...
The resident runtime contracts are deliberately non-portable. Their source
declarations resolve to full contract addresses in component provenance; the
table does not show persisted artifact identities.

| Runtime contract | Source declaration | Artifact policy |
| --- | --- | --- |
...
```text
uv run --extra dev python scripts/profile_branch_search_resident_authority.py
```

The resident and packed-layout profilers are reproducibility tools, not public
hardware claims. They report transfer volume, kernel launches, synchronizations
and allocator deltas for the local machine that runs them. Packed execution may
reduce the active work surface, but its gather/scatter and launch overhead must
be measured on the deployment hardware before claiming a speedup.
...
The Formula/Topology control runs the same frozen candidates through Recall,
Formula, and `Fold@2 -> ADD Formula -> UnFold@2` plans. It verifies that the
composed executor applies a topology-dependent transition and records cost; it
does not prove downstream quality or acceleration.
