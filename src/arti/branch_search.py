"""Versioned Recall Top-K branch batches for true K-wide branch search."""
...
def _canonical_component_ref(value: object) -> str:
    """Resolve a runtime component to its artifact-safe contract reference."""
...
def _canonical_contract_ref(reference: str) -> str:
    """Resolve a declaration to the immutable component contract it names."""
...
def _canonical_formula_ref(reference: str) -> str:
    """Resolve a Formula declaration without applying component contract hashing."""
    ...
    try:
        ...
        raise BranchSearchContractError(
            "formula_ref must resolve to an explicitly registered immutable Formula contract"
        ) from error


class BranchSearchContractError(ValueError):
...
_RNG_PHASES = frozenset(
    {"candidate-route", "iteration-route", "half-survival", "recall-dropout"}
)
...
class ExecutionRNGPlan:
    ...
    _runtime_contract_ref: ClassVar[str] = "arti/execution-rng-plan@3"
    ...
    def contract_ref(self) -> str:
        """Immutable schema identity for this runtime-only execution plan."""

        return _canonical_contract_ref(self._runtime_contract_ref)
    ...
    def _derived_seed(
        ...
        *,
        ...
        if phase not in _RNG_PHASES:
            raise BranchSearchContractError(f"unsupported execution RNG phase {phase!r}")
        payload = json.dumps(
            {
                "algorithm": self.algorithm,
                "seed": self.seed,
    ...
    def bind(self, branch_origin_index: Tensor) -> "_KeyedBranchRandomSource":
        ...
        if branch_origin_index.shape[0] != len(self.sample_keys):
            raise BranchSearchContractError(
                "execution RNG sample_keys must match the Branch search batch"
            )
...
class _KeyedBranchRandomSource:
    ...
    def select_rows(self, flat_index: Tensor) -> "_KeyedBranchRandomSource":
        ...
        if not isinstance(flat_index, Tensor) or flat_index.dtype != torch.long:
            raise BranchSearchContractError("packed RNG row index must be torch.long")
    ...
    def uniform(
        ...
        if not isinstance(iteration_index, int) or iteration_index < 0:
            raise BranchSearchContractError("iteration_index must be non-negative")
...
class BranchSearchOperation(nn.Module):
    """Versioned adapter for an existing Formula or Topology operation."""

    _component_reference: ClassVar[str] = "arti/branch-search-operation@1"
    ...
    def __init__(self, operation: nn.Module) -> None:
        ...
        if not isinstance(
            ...
            raise BranchSearchContractError(
                "BranchSearchOperation supports existing FormulaResidentOperation "
                "or TopologyFormulaResidentOperation only"
            )
        self.operation = operation
        self.operation_ref = _canonical_component_ref(operation)
    ...
    def _capture_operation_lineage(self) -> None:
        ...
        if fold is not None:
            topology_refs.append(_canonical_component_ref(fold))
            topology_refs.append(_canonical_component_ref(fold.topology))
            topology_contract_fingerprints.append(
                fold.topology.contract_fingerprint
            )
        if unfold is not None:
            topology_refs.append(_canonical_component_ref(unfold))
            topology_contract_fingerprints.append(
        ...
        self.config_fingerprint = _config_fingerprint(
            {
                ...
                "resident_iteration_steps": operation.resident_iteration_steps,
    ...
    def work_per_execution(
        ...
        multiplier = self.operation.resident_iteration_steps
    ...
    def indexed(
        ...
        *,
        ...
        return _BranchIndexedBranchSearchOperation(
            self,
            branch_origin_index,
    ...
    def assert_unchanged(self) -> None:
        ...
        if set(current) != {name for name, _, _ in self._lineage}:
            raise BranchSearchContractError("Branch search operation lineage changed")
        for name, token, version in self._lineage:
            value = current[name]
            if id(value) != token or _tensor_version(value) != version:
                raise BranchSearchContractError(
                    "Branch search operation changed after plan construction"
                )
...
class _BranchIndexedBranchSearchOperation(nn.Module):
    """Runtime-only Formula/Topology view aligned to candidate branch origins."""

    def __init__(
        self,
        source: BranchSearchOperation,
        branch_origin_index: Tensor,
        *,
...
@dataclass(frozen=True)
class BranchSearchPlan:
    """Versioned composition plan for K Recall trajectories."""

    operation: BranchSearchOperation | None = None
    execution_layout: str = "static_capacity"
    schema_version: int = 1
    _component_reference: ClassVar[str] = "arti/branch-search-plan@1"

    def __post_init__(self) -> None:
        if self.schema_version != 1:
            raise BranchSearchContractError("unsupported BranchSearchPlan schema")
        if self.operation is not None and not isinstance(
            self.operation, BranchSearchOperation
        ):
            raise TypeError("operation must be BranchSearchOperation or None")
    ...
    def compose(
        ...
        *,
        ...
        return cls(
            BranchSearchOperation(operation),
...
def _assert_tensor(condition: Tensor, message: str) -> None:
    ...
    if scalar.device.type == "cpu":
        if not bool(scalar):
            raise BranchSearchContractError(message)
        return
...
def _versioned_runtime_tensor(value: Tensor) -> Tensor:
    ...
    if _tensor_version(snapshot) < 0:  # pragma: no cover - defensive boundary
        raise BranchSearchContractError("failed to create versioned runtime storage")
    return snapshot
...
def _source_config_fingerprint(
    ...
    *,
    ...
    activation_config = {
        "ref": _canonical_contract_ref(
            getattr(activation, "_component_reference", "torch/identity@1")
        ),
    ...
    candidate_ref = _canonical_contract_ref(
    ...
    return _config_fingerprint(
        {
            "ref": candidate_ref,
            "source_ref": _canonical_component_ref(recall),
...
def _recall_formula_reference(field: object) -> str:
    from .recall_formula import builtin_formula_reference

    composition = field.value_composition
    if composition == "single":
        return builtin_formula_reference("single")
    if composition == "product":
        return builtin_formula_reference("product")
    if composition == "state":
        return builtin_formula_reference("state")
    if composition != "custom":
        raise BranchSearchContractError("unsupported Recall value composition")
...
def _partition_contract(
    ...
    fingerprint = _config_fingerprint(
        {
            "ref": _canonical_contract_ref("arti/recall-partition-layout@1"),
            "routing_normalizer": field.routing_normalizer,
            "names": names,
            "ranges": ranges,
...
def _resolve_single_candidate_allocation(
    ...
    *,
    ...
    if not enabled:
        raise BranchSearchContractError("candidate allocation has no enabled Bank")
    if allocation == "explicit_bank_quota":
        ...
        if sum(quotas) != resolved_k:
            raise BranchSearchContractError("bank_quotas must sum to max_k")
        if any(quota > capacity for quota, capacity in zip(quotas, capacities, strict=True)):
            raise BranchSearchContractError("a Bank quota exceeds its route capacity")
        if any(quota and weights[index] == 0.0 for index, quota in enumerate(quotas)):
            raise BranchSearchContractError("a zero-weight Bank must have zero quota")
    ...
    while remaining and any(quotas[index] < capacities[index] for index in enabled):
        ...
        if not progressed:  # pragma: no cover - guarded by capacity validation
            raise BranchSearchContractError("unable to allocate the requested Bank quota")
...
def _select_partition_quota_groups(
    ...
    *,
    ...
    if not ordered:
        raise BranchSearchContractError("candidate allocation selected no routes")
...
def _joint_factor_topk(route_mass: Tensor, output_k: int) -> tuple[Tensor, Tensor]:
    ...
    if route_mass.ndim != 4 or output_k < 1:
        raise BranchSearchContractError("joint factor route mass has an invalid shape")
...
def _gather_factor_candidates(value: Tensor, factor_rank: Tensor) -> Tensor:
    ...
    if value.ndim < 4 or factor_rank.ndim != 4:
        raise BranchSearchContractError("factor candidate gather rank is invalid")
    if value.shape[:3] != (
        factor_rank.shape[0],
        factor_rank.shape[1],
        factor_rank.shape[3],
    ):
        raise BranchSearchContractError("factor candidate gather shape is invalid")
...
def _same_bank_formula_candidates(
    ...
    *,
    ...
    if len(order) != sum(partition_quota):
        raise BranchSearchContractError("same-Bank Formula allocation is incomplete")
    ...
    for value_index in range(6):
        ...
        for partition, rank in order:
            ...
            if piece is None:  # pragma: no cover - order excludes zero quotas
                raise BranchSearchContractError("missing same-Bank Formula candidate")
...
class RecallBranchBatch:
    ...
    def __post_init__(self) -> None:
        if self.schema_version not in {3, 6}:
            raise BranchSearchContractError("unsupported Recall branch-batch schema")
        ...
        if self.routing_normalizer not in {"global", "per_bank"}:
            raise BranchSearchContractError("invalid Recall routing normalizer")
        expected_policies = (
            {
        ...
        if not composed and self.factor_count != 1:
            raise BranchSearchContractError("single Recall must expose one factor")
        ...
        if not isinstance(self.formula_ref, str) or not self.formula_ref:
            raise BranchSearchContractError("formula_ref must be non-empty")
        object.__setattr__(self, "formula_ref", _canonical_formula_ref(self.formula_ref))
        if not isinstance(self.topology_lineage, tuple) or any(
            ...
            raise BranchSearchContractError("topology_lineage must contain strings")
        ...
        if not isinstance(self.source_ref, str) or not self.source_ref:
            raise BranchSearchContractError("source_ref must be non-empty")
        for value, name in (
        ...
        if self.input_shape not in {
            ...
            raise BranchSearchContractError("input_shape does not match branch candidates")
        if not self.input_dtype or not self.input_device:
            raise BranchSearchContractError("input dtype and device must be recorded")
        if self.input_version < 0:
            ...
            if (
                ...
                raise BranchSearchContractError("input snapshot contract is invalid")
        elif (
        ...
        for value, name in (
            ...
            if (
                ...
                raise BranchSearchContractError(f"{name} must be lowercase SHA-256 hex")
        ...
        for value, name in (
            ...
            if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
                raise BranchSearchContractError(f"{name} must be a positive integer")
        if (
        ...
        for name, (start, stop) in zip(
            ...
            if not isinstance(name, str) or not name or start != cursor or stop <= start:
                raise BranchSearchContractError("invalid candidate partition layout")
            cursor = stop
        ...
        if not composed and self.max_k > self.source_topk:
            raise BranchSearchContractError("single Recall max_k must not exceed source_topk")
        ...
        if branches != self.max_k:
            raise BranchSearchContractError("candidate group K axis must equal max_k")
        if composed and self.candidate_group_index.shape[3] != self.factor_count:
            raise BranchSearchContractError("candidate factor axis must equal factor_count")
        if (
        ...
        for value, shape, name in (
            ...
            if not isinstance(value, Tensor) or value.dtype != torch.bool or value.shape != shape:
                raise BranchSearchContractError(f"{name} must be boolean {shape}")
        if (
        ...
        if (
            ...
            raise BranchSearchContractError("active_k must be torch.long [B]")
        if (
        ...
        if any(value.device != device for value in tensors):
            raise BranchSearchContractError("branch-batch tensors must use one device")
    ...
    def source_state(self, value: Tensor) -> Tensor:
        ...
        if self.input_version >= 0:
            ...
            if _tensor_version(self.input_snapshot) != self.input_snapshot_version:
                raise BranchSearchContractError("input snapshot changed after query")
    ...
    def assert_unchanged(self) -> None:
        ...
        if tuple(id(tensor) for tensor in tensors) != self.candidate_tensor_tokens:
            raise BranchSearchContractError("candidate bridge tensor identity changed")
        current_versions = tuple(_tensor_version(tensor) for tensor in tensors)
        if current_versions != self.candidate_tensor_versions:
            raise BranchSearchContractError("candidate bridge tensor changed after query")
    ...
    def canonical_manifest_fingerprint(self) -> str:
        ...
        payload = {
            "ref": _canonical_contract_ref("arti/recall-candidate-manifest@2"),
...
class BranchSearchPolicy:
    ...
    _component_reference: ClassVar[str] = "arti/branch-search-policy@1"
    ...
    def __init__(
        ...
        *,
        ...
        from .execution import AdaptiveExecutionPolicy

        if not isinstance(candidates, RecallBranchBatch):
            raise TypeError("candidates must be RecallBranchBatch")
        if not isinstance(base, AdaptiveExecutionPolicy):
            raise TypeError("base must be AdaptiveExecutionPolicy")
        ...
        self.config_fingerprint = _config_fingerprint(
            {
                "ref": _canonical_contract_ref(self._component_reference),
                "base_ref": _canonical_contract_ref(base._component_reference),
                "base": {
    ...
    def assert_matches(self, candidates: RecallBranchBatch) -> None:
        ...
        for value, (token, version) in zip(
            ...
            if id(value) != token or _tensor_version(value) != version:
                raise BranchSearchContractError("branch policy changed after construction")

    def bind(self, candidates: RecallBranchBatch) -> object:
        """Return a flattened runtime schedule in current candidate order."""

        from .execution import AdaptiveExecutionSchedule
        ...
        return AdaptiveExecutionSchedule(
...
def query_recall_branches(
    ...
    *,
    ...
    Formula, iterate a branch, score a future, or authorize a persistent write.
    """
    ...
    if value.ndim not in {2, 3} or value.shape[-1] != recall.dim:
        raise BranchSearchContractError("value must have shape [B,D] or [B,N,D]")
    field = recall.state.recall
    if field.routing != "grouped":
        raise BranchSearchContractError("Branch search requires grouped Recall routing")
    if field.training and field._training_group_partitions is not None:
        raise BranchSearchContractError(
            "Branch search does not support partition-restricted training routes"
        )
    available_k = field.group_topk
    ...
    if active_k is None:
        ...
        raise BranchSearchContractError("active_k must be an integer or torch.long [B]")
    ...
    if bool(getattr(activation, "survival_runtime_only", False)):
        raise BranchSearchContractError(
            "Branch search requires a versioned portable Half survival"
        )
    route_exploration = (
...
class BranchSearchResult:
    ...
    schema_version: int = 1
    _component_reference: ClassVar[str] = "arti/branch-search-result@1"
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _BATCHED_RESULT_FACTORY_TOKEN:
            raise BranchSearchContractError(
                "BranchSearchResult must come from run_branch_search"
            )
        ...
        object.__setattr__(
            self,
            "value_version",
            _require_tensor_version(value, name="Branch search value"),
        )
        object.__setattr__(
            self,
            "delta_version",
            _require_tensor_version(delta, name="Branch search delta"),
        )
    ...
    def __post_init__(self) -> None:
        if self.schema_version != 1:
            raise BranchSearchContractError("unsupported Branch search result schema")
        if not self.plan_ref.startswith("arti/branch-search-plan@sha256:"):
            raise BranchSearchContractError("unsupported Branch search plan reference")
        if self.execution_layout not in {"static_capacity", "packed_active"}:
            raise BranchSearchContractError(
                "unsupported Branch search execution layout"
            )
        if (
            ...
            raise BranchSearchContractError("invalid Branch search plan fingerprint")
        if self.formula_route_fingerprint is not None and (
            ...
            raise BranchSearchContractError("invalid Formula route fingerprint")
        if any(not isinstance(item, str) or not item for item in self.topology_refs):
            raise BranchSearchContractError("invalid topology component reference")
        ...
        if self.branch_policy_fingerprint is not None and (
            ...
            raise BranchSearchContractError("invalid branch policy fingerprint")
        if self.execution_rng_fingerprint is not None and (
            ...
            raise BranchSearchContractError("invalid execution RNG fingerprint")
        ...
        if self.execution_rng_stream_key is not None and (
            ...
            raise BranchSearchContractError("invalid execution RNG stream key")
        if (
            ...
            raise BranchSearchContractError("invalid execution RNG domain receipt")
        ...
        if (
            ...
            raise BranchSearchContractError("value must be floating [B,K,N,D]")
        if not isinstance(self.delta, Tensor) or self.delta.shape != self.value.shape:
            raise BranchSearchContractError("delta must match the branch value tensor")
        if self.delta.device != self.value.device:
            raise BranchSearchContractError("result tensors must use one device")
    ...
    def assert_unchanged(self) -> None:
        ...
        for tensor, expected in (
            ...
            if _tensor_version(tensor) != expected:
                raise BranchSearchContractError(
                    "Branch search result changed before staging"
                )
        current_diagnostics = {
        ...
        if set(current_diagnostics) != {name for name, _, _ in self.diagnostic_lineage}:
            raise BranchSearchContractError("Branch search diagnostics changed before staging")
        for name, token, version in self.diagnostic_lineage:
            ...
            if id(tensor) != token or _tensor_version(tensor) != version:
                raise BranchSearchContractError(
                    "Branch search diagnostics changed before staging"
                )
...
def run_branch_search(
    ...
    *,
    ...
    plan: BranchSearchPlan | None = None,
    rng_plan: ExecutionRNGPlan | None = None,
    ...
    plan = BranchSearchPlan.recall_only() if plan is None else plan
    if not isinstance(plan, BranchSearchPlan):
        raise TypeError("plan must be BranchSearchPlan or None")
    ...
    if candidates.query_rng_fingerprint is not None and (
        ...
        raise BranchSearchContractError(
            "candidate query RNG does not match the Branch search execution plan"
        )
    field = recall.state.recall
    if candidates.source_ref != _canonical_component_ref(recall):
        raise BranchSearchContractError("candidate source does not match Recall")
    if candidates.source_instance_token != id(field):
        raise BranchSearchContractError("candidate source instance does not match Recall")
    if candidates.layout_fingerprint != field._route_layout_fingerprint():
        raise BranchSearchContractError("candidate layout does not match Recall")
    ...
    if candidate_versions != current_versions:
        raise BranchSearchContractError("candidate source changed after its Recall query")
    ...
    branch_policy = execution_policy if isinstance(execution_policy, BranchSearchPolicy) else None
    policy = (
        branch_policy.base
        if branch_policy is not None
        else execution_policy or ExecutionPolicy.fixed(1, trace_level="routes")
    )
    if not isinstance(policy, (ExecutionPolicy, AdaptiveExecutionPolicy)):
        raise TypeError("execution_policy must be ExecutionPolicy or AdaptiveExecutionPolicy")
    if isinstance(policy, AdaptiveExecutionPolicy) and policy.executor != "static_masked":
        raise BranchSearchContractError(
            "Branch search requires static_masked execution to avoid host synchronization"
        )
    if refine_exit is not None and model_exit:
        if not isinstance(policy, AdaptiveExecutionPolicy):
            raise BranchSearchContractError(
                "refine_exit requires AdaptiveExecutionPolicy@2"
            )
    ...
    if active_flat_index is not None:
        ...
        if execution_schedule is not None:
            ...
            execution_schedule = AdaptiveExecutionSchedule(
                min_steps=select_schedule(execution_schedule.min_steps),
                max_steps=select_schedule(execution_schedule.max_steps),
                absolute_tolerance=select_schedule(
                    execution_schedule.absolute_tolerance
                ),
                relative_tolerance=select_schedule(
                    execution_schedule.relative_tolerance
                ),
                patience=select_schedule(execution_schedule.patience),
                route_tolerance=select_schedule(execution_schedule.route_tolerance),
                cycle_tolerance=select_schedule(execution_schedule.cycle_tolerance),
            )
    if physical_rows:
        ...
        iteration_steps = policy.max_steps
        token_shape = (0, tokens)
        token_step_shape = (0, iteration_steps, tokens)
        diagnostics = {
            ...
            "recall_token_stop_reason": torch.full(
                token_shape,
                int(ExecutionStopReason.MASKED),
                device=source_value.device,
                dtype=torch.int64,
            ),
            ...
            "recall_active_fraction": source_value.new_zeros((iteration_steps,)),
            ...
            "recall_step_attempted": torch.zeros(
                (0, iteration_steps), device=source_value.device, dtype=torch.bool
            ),
            "recall_step_committed": torch.zeros(
                (0, iteration_steps), device=source_value.device, dtype=torch.bool
            ),
            "recall_step_update_ratio": source_value.new_zeros((0, iteration_steps)),
            "recall_step_route_change": source_value.new_zeros((0, iteration_steps)),
    ...
    if kernel_steps is None and isinstance(policy, ExecutionPolicy):
    ...
    if plan.operation is not None and physical_rows:
        if not isinstance(kernel_steps, Tensor) or kernel_steps.numel() != 1:
            raise BranchSearchContractError(
                "composed Branch search requires a scalar kernel-step receipt"
            )
        resident_steps = plan.operation.operation.resident_iteration_steps
        ...
        if attempted_trace is None or attempted_trace.shape[:3] != (
            ...
            raise BranchSearchContractError(
                "composed Branch search requires [B,K,R,...] attempt trace"
            )
        ...
        resident_steps = operation.resident_iteration_steps
    ...
    if attempted_steps is None or committed_steps is None:
        raise BranchSearchContractError(
            "Branch search requires attempted and committed step diagnostics"
        )
    ...
    return BranchSearchResult(
        ...
        plan_ref=_canonical_component_ref(plan),
        plan_config_fingerprint=plan.config_fingerprint,
        execution_layout=plan.execution_layout,
        operation_ref=(
            None
            if plan.operation is None
            else _canonical_component_ref(plan.operation.operation)
        ),
        formula_route_fingerprint=(
            None
            if plan.operation is None
