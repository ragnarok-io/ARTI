"""Versioned contracts shared by the composable ARTI runtime.
...
def _canonical_contract_ref(reference: str) -> str:
    """Resolve convenience declarations before they enter runtime metadata."""

    # Compiled support wrappers are ephemeral graph values, not artifact
    # metadata. Registry resolution here would introduce a Python lock into a
    # captured CUDA graph; eager construction remains the serialization gate.
...
class _JsonBudget:
    ...
    def normalize(self, value: Any, *, depth: int = 0) -> Any:
        ...
        if self.nodes > self.limits.max_json_nodes:
            raise ValueError("runtime config exceeds max_json_nodes")
        if depth > self.limits.max_json_depth:
            raise ValueError("runtime config exceeds max_json_depth")
        if isinstance(value, Mapping):
            if len(value) > self.limits.max_json_nodes:
                raise ValueError("runtime config mapping exceeds max_json_nodes")
            ...
            if any(
                ...
                raise ValueError("runtime contract key exceeds max_string_bytes")
            return {
        ...
        if isinstance(value, (str, bool, int)) or value is None:
            if isinstance(value, str) and len(value.encode("utf-8")) > self.limits.max_string_bytes:
                raise ValueError("runtime contract string exceeds max_string_bytes")
            return value
        if isinstance(value, float):
            if not math.isfinite(value):
                raise ValueError("runtime contract floats must be finite")
            return value
        raise TypeError(f"unsupported runtime contract value: {type(value).__name__}")
...
def _canonical_json(value: Any, limits: ContractLimits = DEFAULT_CONTRACT_LIMITS) -> str:
    ...
    if len(encoded.encode("utf-8")) > limits.max_config_bytes:
        raise ValueError("runtime config exceeds max_config_bytes")
    return encoded
...
class SupportDomain:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "owner_ref", _canonical_contract_ref(self.owner_ref))
    ...
    def metadata(self) -> dict[str, Any]:
        return {
            ...
            "ref": _canonical_contract_ref(self._runtime_contract_ref),
...
class OperandContract:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "source_ref", _canonical_contract_ref(self.source_ref))
        object.__setattr__(self, "consumer_ref", _canonical_contract_ref(self.consumer_ref))
...
def _envelope_contract_ref(reference: EnvelopeRef) -> str:
    """Serialize an execution-only envelope enum as an immutable contract."""

    return _canonical_contract_ref(reference.value)


def _envelope_ref_from_contract(reference: object) -> EnvelopeRef:
    """Resolve a persisted envelope contract back to its execution enum."""

    if not isinstance(reference, str):
        raise TypeError("envelope reference must be a string")
    for candidate in EnvelopeRef:
        if reference == _envelope_contract_ref(candidate):
            return candidate
    raise ValueError("unsupported envelope contract reference")
...
class TopologyBinding:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "topology_ref",
            _canonical_contract_ref(self.topology_ref),
        )
        object.__setattr__(
            self,
            "producer_ref",
            _canonical_contract_ref(self.producer_ref),
        )
        object.__setattr__(
            self,
            "inverse_ref",
            _canonical_contract_ref(self.inverse_ref),
        )
        ...
        if self.topology_ref != _canonical_contract_ref("arti/reversible-topology@1"):
            raise ValueError("TopologyBinding@1 requires ReversibleTopology@1")
        if (
            self.producer_ref != _canonical_contract_ref("arti/fold@2")
            or self.inverse_ref != _canonical_contract_ref("arti/unfold@2")
        ):
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "TopologyBinding":
        ...
        if value["ref"] != _canonical_contract_ref(cls._runtime_contract_ref):
    ...
    def validate_record(self, record: object) -> None:
        ...
        for name, value in expected.items():
            record_value = getattr(record, name, None)
            actual = (
                _canonical_contract_ref(record_value)
                if name.endswith("_ref") and isinstance(record_value, str)
                else record_value
            )
            if actual != value:
...
class PulseStageSpec:
    ...
    def __init__(
        ...
        *,
        ...
        if mode is StageMode.OFF:
            ...
            component_ref = _canonical_contract_ref(component_ref)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "input_schema": _envelope_contract_ref(self.input_schema),
            "output_schema": _envelope_contract_ref(self.output_schema),
            "config": self.config,
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "PulseStageSpec":
        ...
        if (
            ...
            or value["ref"] != _canonical_contract_ref(cls._component_reference)
        ):
        ...
        return cls(
            ...
            input_schema=_envelope_ref_from_contract(value["input_schema"]),
            output_schema=_envelope_ref_from_contract(value["output_schema"]),
