...
# The execution vocabulary is the canonical public surface.  The Refine names
# remain below as implementation/legacy contracts so existing artifacts retain
# their exact numerical semantics while new program code does not imply that
# every iteration is a tensor repair operation.
@dataclass(frozen=True)
class ExecutionBudget(RefineBudget):
    """Bound the iterations available to one program execution."""

    _component_reference: ClassVar[str] = "arti/execution-budget@1"


@dataclass(frozen=True)
class ExecutionStop(RefineStop):
    """Stopping rules for an adaptive program execution."""

    _component_reference: ClassVar[str] = "arti/execution-stop@1"


@dataclass(frozen=True)
class AdaptiveExecutionPolicy(AdaptiveRefinePolicy):
    """Composable adaptive execution policy.

    This is intentionally a subtype of :class:`AdaptiveRefinePolicy`: engines
    that already implement the numerical contract accept it without a wrapper.
    """
...
@dataclass(frozen=True)
class ExecutionPolicy(RefinePolicy):
    """Per-call execution policy for an iterative program."""
...
# Trace values describe execution, not an obligation to improve a state.
ExecutionStopReason = RecallStopReason
ExecutionTrace = RecallTrace
ExecutionTraceV2 = RecallTraceV2
ExecutionTraceV3 = RecallTraceV3
