...
def _require_component_ref(value: str, name: str) -> str:
    """Accept an explicit declaration, but retain only its full contract address."""
...
class BranchBudget:
    """Bounded iteration work allowed for one branch."""
...
class BranchBatchSpec:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "executor_ref", _require_component_ref(self.executor_ref, "executor_ref")
        )
        for value, name in (
...
class BranchBatchSpecV2:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "scorer_ref", _require_component_ref(self.scorer_ref, "scorer_ref")
        )
        for value, name in (
    ...
    @classmethod
    def from_branch_search(
        cls,
        snapshot: TensorSnapshot,
        executor: "BranchSearchExecutor",
        *,
        run_id: str,
        branch_ids: Sequence[str],
        input_fingerprint: str,
        ...
        if not isinstance(executor, BranchSearchExecutor):
            raise TypeError("executor must be BranchSearchExecutor")
...
class OverlayProposal:
    ...
    def __post_init__(self) -> None:
        ...
        if step_inputs[1:] != step_outputs[:-1]:
            raise TensorTransactionContractError(
                "each iteration must read the preceding iteration output"
            )
...
class BranchSearchOverlayProposal:
    """Factory-owned CPU proposal for one real Branch search trajectory."""
    ...
    work: BranchWorkReceipt
    _delta: Tensor
    _runtime_contract_ref: ClassVar[str] = "arti/branch-search-overlay@1"
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _BATCHED_OVERLAY_FACTORY_TOKEN:
            raise TensorTransactionContractError(
                "BranchSearchOverlayProposal must come from stage_branch_search_result"
            )
        _require_identifier(branch_id, "branch_id")
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _BATCH_SCORE_FACTORY_TOKEN:
            raise TensorTransactionContractError(
                "BranchScoreBatchReceipt must come from score_branch_search_proposals"
            )
        ...
        scorer_ref = _require_component_ref(scorer_ref, "scorer_ref")
...
def _work_receipt_payload(value: BranchWorkReceipt) -> dict[str, object]:
    return {
        "ref": canonical_contract_reference(value._runtime_contract_ref),
    ...
    def __init__(
        ...
        *,
        ...
        content = {
            ...
            "rollback_receipts": [
                {
                    "ref": canonical_contract_reference(item._runtime_contract_ref),
...
class K2BranchHarness:
    ...
    def propose(self, proposal: OverlayProposal) -> None:
        ...
        if proposal.step_input_fingerprints[0] != self.spec.input_fingerprint:
            raise TensorTransactionContractError("first iteration does not bind the shared input")
...
_BRANCH_SEARCH_EXECUTOR_FACTORY_TOKEN = object()
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _EXECUTION_CONTEXT_RECEIPT_FACTORY_TOKEN:
            raise TensorTransactionContractError(
                "ExecutionContextReceipt must come from a completed Branch search run"
            )
        if schema_version != 3:
            raise TensorTransactionContractError(
...
def frozen_mse_scorer_config_fingerprint() -> str:
    ...
    return _fingerprint(
        {
            "ref": _require_component_ref(FROZEN_MSE_SCORER_REF, "scorer_ref"),
...
def _branch_search_result_state_fingerprint(
    result: object,
    manifest_fingerprint: str,
) -> str:
    from .branch_search import BranchSearchResult

    if not isinstance(result, BranchSearchResult):
        raise TypeError("result must be BranchSearchResult")
    ...
    return _fingerprint(
        {
            "ref": canonical_contract_reference("arti/branch-search-executor-state@1"),
...
def _resident_branch_search_manifest_fingerprint(result: object) -> str:
    ...
    return _fingerprint(
        {
            "ref": canonical_contract_reference("arti/resident-branch-search-manifest@1"),
...
def _resident_branch_search_state_fingerprint(
    result: object,
    manifest_fingerprint: str,
) -> str:
    ...
    return _fingerprint(
        {
            "ref": canonical_contract_reference(
                "arti/resident-branch-search-executor-state@1"
            ),
            "candidate_manifest_fingerprint": manifest_fingerprint,
...
class BranchSearchExecutor:
    ...
    identity_mode: str
    _component_reference: ClassVar[str] = "arti/branch-search-executor@1"
    ...
    def __init__(
        ...
        *,
        ...
        if _factory_token is not _BRANCH_SEARCH_EXECUTOR_FACTORY_TOKEN:
            raise TensorTransactionContractError(
                "BranchSearchExecutor must come from BranchSearchExecutor.from_result"
            )
        if schema_version != 1:
            raise TensorTransactionContractError(
                "unsupported BranchSearchExecutor schema"
            )
        candidate_ref = _require_component_ref(candidate_ref, "candidate_ref")
        plan_ref = _require_component_ref(plan_ref, "plan_ref")
        if operation_ref is not None:
            operation_ref = _require_component_ref(operation_ref, "operation_ref")
        if execution_layout not in {"static_capacity", "packed_active"}:
            raise TensorTransactionContractError(
                "unsupported BranchSearchExecutor execution layout"
            )
        for value, name in (
            (candidate_manifest_fingerprint, "candidate_manifest_fingerprint"),
            (plan_config_fingerprint, "plan_config_fingerprint"),
        ...
        if identity_mode not in {"content", "resident_identity"}:
            raise TensorTransactionContractError(
                "unsupported BranchSearchExecutor identity mode"
            )
    ...
    @classmethod
    def from_result(cls, result: object) -> "BranchSearchExecutor":
        from .branch_search import BranchSearchResult
        from .component_registry import component_ref

        if not isinstance(result, BranchSearchResult):
            raise TypeError("result must be BranchSearchResult")
        manifest = branch_search_manifest_fingerprint(result)
        ...
        return cls(
            ...
            state_fingerprint=_branch_search_result_state_fingerprint(
                result,
                manifest,
            ),
            identity_mode="content",
            _factory_token=_BRANCH_SEARCH_EXECUTOR_FACTORY_TOKEN,
        )

    @classmethod
    def from_resident_result(cls, result: object) -> "BranchSearchExecutor":
        ...
        if result.value.device.type != "cuda":
            raise TensorTransactionContractError(
                "resident executor requires a CUDA Branch search result"
            )
        manifest = _resident_branch_search_manifest_fingerprint(result)
        route_fingerprint = result.formula_route_fingerprint or ("0" * 64)
        context_factory = (
            ExecutionContextReceipt.deterministic
            if result.execution_rng_fingerprint is None
        ...
        return cls(
            ...
            branch_policy_fingerprint=result.branch_policy_fingerprint,
            execution_context=execution_context,
            state_fingerprint=_resident_branch_search_state_fingerprint(
                result,
                manifest,
            ),
    ...
    def assert_matches(self, result: object) -> None:
        ...
        if not isinstance(result, BranchSearchResult):
            raise TypeError("result must be BranchSearchResult")
        manifest = (
            branch_search_manifest_fingerprint(result)
            if self.identity_mode == "content"
            else _resident_branch_search_manifest_fingerprint(result)
        )
        route_fingerprint = result.formula_route_fingerprint or ("0" * 64)
        if (
            ...
            or (
                _branch_search_result_state_fingerprint(result, manifest)
                if self.identity_mode == "content"
                else _resident_branch_search_state_fingerprint(result, manifest)
            )
            != self.state_fingerprint
        ):
            raise TensorTransactionContractError(
                "Branch search result does not match its executor identity"
            )
    ...
    def bind(
        ...
        *,
        ...
        role: str,
        authority: object,
        state_schema_ref: str = "arti/branch-search-result@1",
...
def branch_search_manifest_fingerprint(result: object) -> str:
    ...
    return _fingerprint(
        {
            "ref": canonical_contract_reference("arti/branch-search-manifest@2"),
            "source_ref": candidates.source_ref,
            "source_config_fingerprint": candidates.source_config_fingerprint,
            "layout_fingerprint": candidates.layout_fingerprint,
            "max_k": candidates.max_k,
...
def stage_branch_search_result(
    result: object,
    executor: BranchSearchExecutor,
    spec: BranchBatchSpecV2,
    bindings: Sequence[ExternalTensorBinding],
) -> tuple[BranchSearchOverlayProposal, ...]:
    ...
    if len(active_origins) != len(spec.branch_ids):
        raise TensorTransactionContractError(
            "active Branch search K does not match BranchBatchSpec@2"
        )
    if active_origins != list(range(len(spec.branch_ids))):
        raise TensorTransactionContractError(
            "active Branch search origins do not match canonical branch authority"
        )
    manifest = branch_search_manifest_fingerprint(result)
    if manifest != spec.candidate_manifest_fingerprint:
        raise TensorTransactionContractError("Branch search candidates do not match branch spec")
    if (
        spec.executor_ref != _require_component_ref(
            executor._component_reference, "executor_ref"
        )
        or spec.program_fingerprint != executor.plan_config_fingerprint
        or spec.route_fingerprint != executor.route_fingerprint
    ):
        raise TensorTransactionContractError(
            "BranchBatchSpec@2 does not match the Branch search executor"
        )
    ...
    if (
        ...
        raise TensorTransactionContractError(
            "Branch search staging requires committed-step and stop diagnostics"
        )
    ...
    staged: list[BranchSearchOverlayProposal] = []
    for origin in active_origins:
        ...
        executor_ref = _require_component_ref(
            executor._component_reference, "executor_ref"
        )
        if binding.component_ref != executor_ref:
            raise TensorTransactionContractError(
        ...
        if (
            ...
            raise TensorTransactionContractError(
                "binding does not match the Branch search executor identity"
            )
        ...
        if not finite:
            raise TensorTransactionContractError(
                "Branch search output and delta must be finite before staging"
            )
        proposal = ExternalTensorProposal(
            binding,
            output,
            producer_ref=executor_ref,
        ...
        if branch_stop_reason.ndim == 1:
            ...
            raise TensorTransactionContractError(
                "Branch search stop diagnostics have an invalid branch shape"
            )
        ...
        trajectory_fp = _fingerprint(
            {
                "ref": canonical_contract_reference("arti/branch-search-trajectory@1"),
...
def score_branch_search_proposals(
    proposals: Sequence[BranchSearchOverlayProposal],
    future: Tensor,
    spec: BranchBatchSpecV2,
    ...
    if (
        spec.scorer_ref != _require_component_ref(
            FROZEN_MSE_SCORER_REF, "scorer_ref"
        )
        or spec.scorer_config_fingerprint
        != frozen_mse_scorer_config_fingerprint()
    ):
        raise TensorTransactionContractError(
            "Branch search scoring requires the canonical frozen MSE scorer"
        )
...
class ResidentBranchRun:
    ...
    def __init__(
        ...
        if executor.identity_mode != "resident_identity":
            raise TensorTransactionContractError(
                "resident branch run requires BranchSearchExecutor.from_resident_result"
            )
        executor.assert_matches(result)
        result.assert_unchanged()
        ...
        if (
            spec.scorer_ref
            != _require_component_ref(FROZEN_MSE_SCORER_REF, "scorer_ref")
            or spec.scorer_config_fingerprint
            != frozen_mse_scorer_config_fingerprint()
        ):
            raise TensorTransactionContractError(
...
def bind_resident_branch_run(
    ...
    """Bind one CUDA Branch search result to resident host authority."""
...
class BranchBatchHarness:
    ...
    def __init__(
        ...
        self._proposals: dict[str, BranchSearchOverlayProposal] = {}
        self._receipt: BranchRunReceipt | None = None
        self._decision_request_fingerprint: str | None = None

    def propose(self, proposal: BranchSearchOverlayProposal) -> None:
        if self._receipt is not None:
            raise TensorTransactionContractError("branch run is already closed")
        if not isinstance(proposal, BranchSearchOverlayProposal):
            raise TensorTransactionContractError("proposal must be a BranchSearchOverlayProposal")
...
__all__ = [
    ...
    "branch_search_manifest_fingerprint",
    "bind_resident_branch_run",
    "score_branch_search_proposals",
    "stage_branch_search_result",
]
