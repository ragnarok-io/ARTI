"""Versioned runtime contracts for bounded iterative execution."""
...
EXECUTION_TRACE_V2_SCHEMA_VERSION = 2
EXECUTION_TRACE_V3_SCHEMA_VERSION = 3
EXECUTION_TRACE_V3_SCHEMA_REF = "arti/execution-trace@3"
...
_EXECUTION_SCOPES = frozenset({"sample", "token"})
_EXECUTION_EXECUTORS = frozenset({"static_masked", "early_break"})
...
class ExecutionStopReason(IntEnum):
    """Tensor-safe reason codes used by :class:`ExecutionTrace`."""
...
_EXECUTION_TRACE_V1_REASONS = (
    ExecutionStopReason.MAX_STEPS,
    ExecutionStopReason.CONVERGED,
    ExecutionStopReason.NONFINITE,
    ExecutionStopReason.CYCLE,
    ExecutionStopReason.MASKED,
)
_EXECUTION_TRACE_V3_REASONS = (*_EXECUTION_TRACE_V1_REASONS, ExecutionStopReason.MODEL_EXIT)
...
class ExecutionPolicy:
    ...
    _component_reference: ClassVar[str] = "arti/execution-policy@1"
    ...
    @classmethod
    def fixed(cls, steps: int, **kwargs: object) -> "ExecutionPolicy":
...
class ExecutionBudget:
    """Versioned deterministic bound for one adaptive execution."""

    _component_reference: ClassVar[str] = "arti/execution-budget@1"
...
@dataclass(frozen=True)
class ExecutionStop:
    """Versioned convergence and cycle rules for adaptive execution."""

    _component_reference: ClassVar[str] = "arti/execution-stop@1"
...
class AdaptiveExecutionPolicy:
    """Composable version-2 runtime policy for adaptive Recall execution.
    ...
    _component_reference: ClassVar[str] = "arti/execution-policy@2"
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.budget, ExecutionBudget):
            raise TypeError("budget must be a ExecutionBudget")
        if not isinstance(self.stop, ExecutionStop):
            raise TypeError("stop must be a ExecutionStop")
        ...
        if self.checkpoint_mode not in _CHECKPOINT_MODES:
            raise ValueError("checkpoint_mode must be 'detached' or 'gradient'")
        if self.executor not in _EXECUTION_EXECUTORS:
    ...
    def replace(self, **changes: object) -> "AdaptiveExecutionPolicy":
...
@dataclass(frozen=True)
class AdaptiveExecutionSchedule:
    """Internal tensor schedule for independently configured execution samples.

    ``AdaptiveExecutionPolicy`` remains the portable, scalar configuration. This
    ...
    _component_reference: ClassVar[str] = "arti/adaptive-execution-schedule@1"
    ...
    def __post_init__(self) -> None:
        ...
        for value, name, required_dtype in tensors:
            ...
            if value.numel() != size or value.device != device:
                raise ValueError("adaptive execution schedule tensors must share shape and device")
        for value, name in (
        ...
        if size is None or size == 0:
            raise ValueError("adaptive execution schedule must contain at least one sample")
...
class RetrievalRoutePlan:
    ...
    def detach(self) -> "RetrievalRoutePlan":
        return replace(
            self,
    ...
    def clone(self) -> "RetrievalRoutePlan":
        return replace(
            self,
...
RetrievalRouteNode: TypeAlias = "RetrievalRoutePlan | RetrievalRouteStack"


@dataclass(frozen=True)
class RetrievalRouteStack:
    ...
    axis: str
    items: tuple[RetrievalRouteNode, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(item, (RetrievalRoutePlan, RetrievalRouteStack)) for item in self.items):
            raise TypeError("route-stack items must be RetrievalRoutePlan or RetrievalRouteStack")

    def detach(self) -> "RetrievalRouteStack":
        return replace(self, items=tuple(item.detach() for item in self.items))

    def clone(self) -> "RetrievalRouteStack":
        return replace(self, items=tuple(item.clone() for item in self.items))

    def to(self, device: torch.device | str) -> "RetrievalRouteStack":
        def move(item: RetrievalRouteNode) -> RetrievalRouteNode:
            if isinstance(item, RetrievalRouteStack):
...
class ExecutionTrace:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != EXECUTION_TRACE_SCHEMA_VERSION:
            raise ValueError("unsupported ExecutionTrace schema version")
        if (
    ...
    def validate(self) -> "ExecutionTrace":
        ...
        valid_reasons = torch.tensor(
            [int(value) for value in _EXECUTION_TRACE_V1_REASONS],
    ...
    def from_diagnostics(
        ...
        *,
        ...
        if validate_schema and (
            ...
            or schema.numel() != 1
            or int(schema.detach().cpu()) != EXECUTION_TRACE_SCHEMA_VERSION
        ):
...
class ExecutionTraceV2:
    ...
    Token counts and masks describe logical iteration. ``kernel_steps``
    ...
    indices: Tensor
    checkpoints: Mapping[int, Tensor]
    schema_version: int = EXECUTION_TRACE_V2_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EXECUTION_TRACE_V2_SCHEMA_VERSION:
            raise ValueError("unsupported ExecutionTraceV2 schema version")
        ...
        iteration_steps = expected[1]
        if self.active_fraction.shape != (iteration_steps,):
        ...
        if any(value.device != reference_device for value in tensors):
            raise ValueError("ExecutionTraceV2 tensors must use one device")
    ...
    def validate(self) -> "ExecutionTraceV2":
        ...
        valid_reasons = torch.tensor(
            [
                ...
                int(ExecutionStopReason.CONVERGED),
                int(ExecutionStopReason.NONFINITE),
                int(ExecutionStopReason.CYCLE),
    ...
    def from_diagnostics(
        ...
        *,
        ...
        if validate_schema and (
            not isinstance(schema, Tensor)
            or schema.numel() != 1
            or int(schema.detach().cpu()) != EXECUTION_TRACE_V2_SCHEMA_VERSION
        ):
            raise ValueError("Recall diagnostics use an unsupported trace schema")
        required = {
...
class ExecutionTraceV3:
    ...
    _component_reference: ClassVar[str] = EXECUTION_TRACE_V3_SCHEMA_REF

    base: ExecutionTraceV2
    token_stop_reason: Tensor
    ...
    schema_version: int = EXECUTION_TRACE_V3_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != EXECUTION_TRACE_V3_SCHEMA_VERSION:
            raise ValueError("unsupported ExecutionTraceV3 schema version")
        if not isinstance(self.base, ExecutionTraceV2):
            raise TypeError("base must be ExecutionTraceV2")
        if (
        ...
        if any(value.device != device for value in values):
            raise ValueError("ExecutionTraceV3 tensors must use the base trace device")

    def __getattr__(self, name: str):
        return getattr(object.__getattribute__(self, "base"), name)

    def validate(self) -> "ExecutionTraceV3":
        """Validate the base trace and neural-control subset relations."""

        self.base.validate()
        valid_reasons = torch.tensor(
            [int(value) for value in _EXECUTION_TRACE_V3_REASONS],
            device=self.token_stop_reason.device,
            dtype=self.token_stop_reason.dtype,
        )
        if not bool(torch.isin(self.token_stop_reason, valid_reasons).all()):
        ...
        projected_reason = torch.where(
            self.token_stop_reason == int(ExecutionStopReason.MODEL_EXIT),
            torch.full_like(
                self.token_stop_reason,
                int(ExecutionStopReason.MAX_STEPS),
            ),
        ...
        reason_is_exit = self.token_stop_reason == int(ExecutionStopReason.MODEL_EXIT)
    ...
    def from_diagnostics(
        ...
        *,
        ...
        if validate_schema and (
            not isinstance(schema, Tensor)
            or schema.numel() != 1
            or int(schema.detach().cpu()) != EXECUTION_TRACE_V3_SCHEMA_VERSION
        ):
            raise ValueError("Recall diagnostics use an unsupported trace schema")
        required = {
        ...
        trace = cls(
            base=ExecutionTraceV2.from_diagnostics(
                legacy_values,
