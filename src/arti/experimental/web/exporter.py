...
def export(
    ...
    *,
    ...
    module_metadata = {
        ...
        "state_contract": component_state_contract(module, module.state_dict(), scope="all"),
    }
    context_contract = _module_context_contract(module)
    if context_contract is not None:
        module_metadata["context_contract"] = context_contract
    manifest = {
...
def _module_config(module: nn.Module):
    ...
    serialization_config = getattr(module, "serialization_config", None)
...
def _module_context_contract(module: nn.Module):
    config = getattr(module, "config", None)
    contract = getattr(config, "context_contract", None)
