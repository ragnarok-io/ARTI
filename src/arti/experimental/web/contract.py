...
def artifact_schema() -> dict[str, object]:
    ...
    tensor = {
        ...
        "properties": {
            ...
            "dtype": {"enum": ["float32", "bool", "int64"]},
            "logical_type": {"enum": ["tensor", "mask", "index"]},
            "shape": {
                "type": "array",
                "minItems": 0,
            ...
            "role": {"enum": ["primary", "workspace", "diagnostic"]},
        },
        "additionalProperties": False,
    }
    return {
                ...
                "properties": {
                    ...
                    "module": {
                        ...
                        "properties": {
                            ...
                            "config": {"type": "object"},
                            "forward_kwargs": {"type": "object"},
                            "context_contract": {"type": "object"},
                            "component_provenance": {"type": "object"},
                            "state_contract": {"type": "object"},
                        },
                        "additionalProperties": False,
                    },
                    "runtime": {
...
def render_typescript_contract() -> str:
    ...
    return '''// Generated by arti.experimental.web.contract. Do not edit by hand.
export type ActiveARTIDevice = 'webgpu' | 'wasm';
export type TensorDType = 'float32' | 'bool' | 'int64';
export type TensorLogicalType = 'tensor' | 'mask' | 'index';
export type OutputRole = 'primary' | 'workspace' | 'diagnostic';
export interface TensorTolerance { atol: number; rtol: number; }
export interface TensorContract {
  name: string; dtype: TensorDType; shape: Array<number | string>;
  logical_type?: TensorLogicalType; dynamic_axes?: Record<string, string>;
  max_bytes?: number; tolerance?: TensorTolerance; role?: OutputRole;
}
export interface ARTIWebManifest {
  ...
  module: {type: string; config: Record<string, unknown>; forward_kwargs?: Record<string, unknown>; context_contract?: Record<string, unknown>; component_provenance?: Record<string, unknown>; state_contract?: Record<string, unknown>};
...
export function parseManifest(value: unknown): ARTIWebManifest {
  ...
  if (module.forward_kwargs !== undefined) record(module.forward_kwargs, 'module forward kwargs');
  if (module.context_contract !== undefined) record(module.context_contract, 'module context contract');
  if (module.component_provenance !== undefined) record(module.component_provenance, 'module component provenance');
  if (module.state_contract !== undefined) record(module.state_contract, 'module state contract');
...
function tensorList(value: unknown, name: string): void {
  ...
  for (const entry of value) {
    ...
    if (typeof item.name !== 'string' || item.name.length === 0 || names.has(item.name) || !['float32', 'bool', 'int64'].includes(String(item.dtype)) || !Array.isArray(item.shape)) throw new Error(`invalid ARTI Web ${name} contract`);
    ...
    if (item.logical_type !== undefined && !['tensor', 'mask', 'index'].includes(String(item.logical_type))) throw new Error(`invalid ARTI Web ${name} logical type`);
    if (item.role !== undefined && !['primary', 'workspace', 'diagnostic'].includes(String(item.role))) throw new Error(`invalid ARTI Web ${name} role`);
    if (item.max_bytes !== undefined && (!Number.isSafeInteger(item.max_bytes) || Number(item.max_bytes) <= 0)) throw new Error(`invalid ARTI Web ${name} byte budget`);
    if (item.tolerance !== undefined) {
      const tolerance = record(item.tolerance, `${name} tolerance`);
      if (!finiteNonNegative(tolerance.atol) || !finiteNonNegative(tolerance.rtol)) throw new Error(`invalid ARTI Web ${name} tolerance`);
    }
    if (item.dynamic_axes !== undefined) {
      const dynamicAxes = record(item.dynamic_axes, `${name} dynamic axes`);
      for (const [axis, symbol] of Object.entries(dynamicAxes)) {
        const index = Number(axis);
        if (!/^(0|[1-9][0-9]*)$/.test(axis) || !Number.isSafeInteger(index) || index >= item.shape.length || typeof symbol !== 'string' || symbol.length === 0 || item.shape[index] !== symbol) throw new Error(`invalid ARTI Web ${name} dynamic axes`);
      }
    }
    names.add(item.name);
  }
...
def render_artifact_typescript(manifest: Mapping[str, object]) -> str:
    ...
    return f'''// Generated from arti-web.json by arti.experimental.web.contract. Do not edit by hand.
