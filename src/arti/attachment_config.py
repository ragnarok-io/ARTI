...
class ARTIAttachTrainingConfig:
    engine: str = "torch"
    objective: str = "tensor_alignment"
    ...
    def __post_init__(self) -> None:
        ...
        if self.objective not in {"tensor_alignment", "model_loss"}:
            raise ValueError("training.objective must be 'tensor_alignment' or 'model_loss'")
...
class ARTIAttachConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.layer:
            raise ValueError("attachment layer configuration must not be empty")

    def to_dict(self, *, include_source: bool = False) -> dict[str, Any]:
        payload = {
            "format_version": self.format_version,
            "layer": _json_value(dict(self.layer)),
...
def load_attach_config(path: str | Path) -> ARTIAttachConfig:
    ...
    if not isinstance(layer, Mapping):
        raise ValueError("attachment TOML requires a [layer] table")
...
def attach_config_from_dict(payload: Mapping[str, Any]) -> ARTIAttachConfig:
    ...
    layer = payload.get("layer")
    if not isinstance(layer, Mapping):
        raise ValueError("attachment config layer value must be a mapping")
...
def write_attach_config(path: str | Path, config: ARTIAttachConfig | None = None) -> Path:
    ...
    if target.suffix.lower() != ".toml":
        raise ValueError("ARTI attachment config must end in .toml")
    value = config or ARTIAttachConfig(layer={"layers": ["model.layers.*"]})
    layer = dict(value.layer)
    lines = ["[arti]", f"format_version = {value.format_version}", "", "[layer]"]
...
def write_attach_lock(
    ...
    *,
    ...
    resolved_layer: Mapping[str, Any],
    host_structure: str,
) -> Path:
    target = Path(path)
    payload = {
        ...
        "resolved_layer": _json_value(dict(resolved_layer)),
